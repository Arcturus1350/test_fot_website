class PomodoroTimer {
    ...
    // 检查背景图片是否能加载，如果失败则使用本地图片
    checkBackgroundImages() {
        // 正常模式背景图片URL
        const normalModeImageUrl = 'https://gss0.baidu.com/9fo3dSag_xI4khGko9WTAnF6hhy/zhidao/pic/item/86d6277f9e2f07088ab175d0e524b899a801f2ae.jpg';
        // 深色模式背景图片URL
        const darkModeImageUrl = 'https://s21.ax1x.com/2025/07/28/pVYeWcT.png';
        
        // 检查正常模式背景图片
        this.checkSingleImage(normalModeImageUrl, (success) => {
            if (!success) {
                console.warn('网络背景图片加载失败，使用本地图片 1.png');
                this.setFallbackBackground('normal');
            }
        });
        
        // 检查深色模式背景图片
        this.checkSingleImage(darkModeImageUrl, (success) => {
            if (!success) {
                console.warn('深色模式网络背景图片加载失败，使用本地图片 2.png');
                this.setFallbackBackground('dark');
            }
        });
    }
    
    // 检查单个图片是否能加载
    checkSingleImage(imageUrl, callback) {
        const img = new Image();
    ...
    // 设置回退背景图片
    setFallbackBackground(mode) {
        const style = document.createElement('style');
        style.textContent = `
            /* 回退背景图片样式 */
            ${mode === 'normal' ? 'body::after' : 'body.dark-mode::after'} {
                background-image: url('./${mode === 'normal' ? '1.png' : '2.png'}') !important;
            }
        `;
        document.head.appendChild(style);
        
        // 显示通知（如果showNotification方法已经可用）
        if (this.showNotification) {
            this.showNotification(`📶 网络图片加载失败，已切换到本地背景图片`);
        }
    }
